let cachedA = eval('$A');
  ...
  export function parseUserInput(value, format){
    ...
    // console.log('localizationService--> ', JSON.stringify(localizationService));
  ...
  export function getWeekdayNames(){
    ...
    console.log('namesOfWeekDays--> ', namesOfWeekDays);

    let days = [];
    if (isNumber(firstDayOfWeek) && getAura().util.isArray(namesOfWeekDays)) {
      console.log('enter in if--> ', firstDayOfWeek);
      ...
      console.log('days after 2nd loop--> ', days);
      return days;
    } 
    else {
      console.log('else names of weekdays--> ', days);
